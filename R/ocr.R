...
pdf_ocr_text <- function(pdf, pages = NULL, opw = "", upw = "", dpi = 600, language = "eng", options = NULL){
  engine <- cpp11tesseract::tesseract(language, options = options)
  images <- pdf_convert(pdf = pdf, pages = pages, opw = opw, upw = upw, dpi = dpi)
  on.exit(unlink(images))
  vapply(images, cpp11tesseract::ocr, character(1), engine = engine, USE.NAMES = FALSE)
}

#' @export
#' @rdname pdf_ocr
pdf_ocr_data <- function(pdf, pages = NULL, opw = "", upw = "", dpi = 600, language = "eng", options = NULL){
  engine <- cpp11tesseract::tesseract(language)
  images <- pdf_convert(pdf = pdf, pages = pages, opw = opw, upw = upw, dpi = dpi)
  on.exit(unlink(images))
  lapply(images, cpp11tesseract::ocr_data, engine = engine)
}
